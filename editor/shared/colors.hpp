...
inline ImU32 to_u32(const glm::vec3& rgb, const float alpha = 1.0f) {
    return ImGui::ColorConvertFloat4ToU32(ImVec4(rgb.x, rgb.y, rgb.z, alpha));
}
inline ImVec4 to_imvec4(const glm::vec4& rgba) {
    return ImVec4(rgba.x, rgba.y, rgba.z, rgba.a);
}
