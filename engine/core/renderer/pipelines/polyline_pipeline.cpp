...
void PolylinePipeline::init(GPUAdapter& gpu) {
    ...
    line_buffer =
        bank.create_buffer(BufferUsage::Vertex | BufferUsage::TransferDst, MAX_POLYLINES, sizeof(PolylineSegment), "Polyline Vertex Buffer").expect("failed to create polyline vertex buffer.");
}
