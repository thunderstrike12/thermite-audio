...
bool Texture2D::fallback(FallbackReason) {
    int tex_width = -1;
    int tex_height = -1;
    int channels = -1;
    unsigned char* data = nullptr;
    ...
    const IO::FileLocation missing_texture_location { IO::Location::ENGINE, "missing.png" };
    data = stbi_load(missing_texture_location.get_absolute_path().string().c_str(), &tex_width, &tex_height, &channels, 4);
    if (!data) {
        return false;
    }
